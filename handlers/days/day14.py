...
async def start_day14(message: types.CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await state.clear()
    question = await message.message.answer("Очень простые и где-то даже тривиальные вещи, к которым мы зачастую относимся несерьезно. На прошлой неделе ты занимался тем, что обращал "
