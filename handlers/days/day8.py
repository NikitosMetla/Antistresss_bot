...
async def start_day8(message: types.CallbackQuery, state: FSMContext, bot: Bot):
    if int(await Users_stat(message.from_user.id).get_user_day()) == int(message.data.split("|")[1]):
        ...
        await message.message.answer(text="Ну что, как успехи? Что стало причиной стресса сегодня?")
...
async def answer_day8_1(message: types.Message, state: FSMContext, bot: Bot):
    await message.answer("Какие мысли при этом возникали?")
...
async def answer_day8_2(message: types.Message, state: FSMContext, bot: Bot):
    await message.answer("А что было с телом?")
...
async def answer_day8_3(message: types.Message, state: FSMContext, bot: Bot):
    await message.answer("Какие эмоции были?")
...
async def answer_day8_4(message: types.Message, state: FSMContext, bot: Bot):
    await message.answer("А что ты делал в этой ситуации (или в этих ситуациях, если их было несколько)?")
