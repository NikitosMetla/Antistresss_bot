...
async def start_day1(message: types.CallbackQuery, state: FSMContext, bot: Bot):
    if int(await Users_stat(message.from_user.id).get_user_day()) == int(message.data.split("|")[1]):
        await state.clear()
        await message.message.answer("Для начала разберёмся, что вообще такое стресс. Что ты понимаешь под стрессом?")
...
@is_now_day(1)
async def answer_start1(message: types.Message, state: FSMContext, bot: Bot):
    await message.answer("Что его вызвало?")
